...
This guide provides step-by-step instructions for deploying the Arbitra backend with load balancing, auto-scaling, and graceful shutdown capabilities.
...
# Verify image
docker images | grep arbitra-backend
```
...
```bash
kubectl create configmap arbitra-backend-config \
  --from-literal=db_host=postgres.staging.svc.cluster.local \
  --from-literal=db_port=5432 \
  --from-literal=db_name=arbitra_staging \
  --from-literal=aws_region=us-east-1 \
  --from-literal=aws_s3_bucket=arbitra-staging \
  -n staging
...
# Watch deployment
kubectl rollout status deployment/arbitra-backend -n staging
...
# Check pod logs
kubectl logs -n staging -l app=arbitra-backend --tail=50

# Check service endpoints
kubectl get endpoints -n staging

# Test health endpoint
kubectl port-forward -n staging svc/arbitra-backend 8080:80 &
...
kubectl create configmap arbitra-backend-config \
  ...
  --from-literal=db_port=5432 \
  --from-literal=db_name=arbitra_production \
  --from-literal=aws_region=us-east-1 \
  --from-literal=aws_s3_bucket=arbitra-production \
  -n production
...
# Inside pod:
wget -O- http://arbitra-backend/health
exit
```
...
```bash
# Get previous image
PREVIOUS_IMAGE=$(kubectl get deployment arbitra-backend -n production -o jsonpath='{.spec.template.spec.containers[0].image}')
...
# Search for warnings
kubectl logs -n production -l app=arbitra-backend | grep WARN
```
