...
This document describes how Arbitra integrates with the Stellar blockchain, including SDK usage, account and wallet management, transaction handling, smart contract interaction, security considerations, error handling, testing, monitoring, troubleshooting, and a checklist.
...
Arbitra uses the Stellar SDK to:
...
If Arbitra interacts with Stellar smart contracts (Soroban), apply the same principles:
...
Arbitra must verify a user controls a wallet address before linking it.
