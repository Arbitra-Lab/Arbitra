...
This document describes the load balancing and auto-scaling setup for the Arbitra backend infrastructure, ensuring high availability and optimal resource utilization in production.
...
The Arbitra backend infrastructure is designed for high availability with:
...
```bash
# Check error logs
kubectl logs -n production -l app=arbitra-backend --tail=100 | grep ERROR
...
kubectl logs -n production -l app=arbitra-backend | grep "duration"
...
kubectl logs -n production -l app=arbitra-backend | grep "connection pool"
...
# Get pod logs
kubectl logs -n production -l app=arbitra-backend --tail=100 -f
...
kubectl get endpoints arbitra-backend -n production

# Port forward for debugging
kubectl port-forward -n production svc/arbitra-backend 3000:80
