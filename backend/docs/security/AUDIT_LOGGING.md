...
This document defines audit logging standards for Arbitra — what events to log,
...
Arbitra's audit logging follows these principles:
...
```logql
{app="arbitra-backend"} | json | level="audit" | event=~"auth\\.login.*"
```
