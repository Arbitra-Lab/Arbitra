...
This document captures the key architecture decisions made for the Arbitra backend, along with the rationale, alternatives considered, and trade-offs accepted for each.
...
The Arbitra platform manages rental agreements, payments, escrow, property listings, blockchain transactions, and user management — domains that share a unified data model and transactional boundaries.
...
Arbitra targets both traditional web users (email/password) and blockchain-native users (Stellar wallets). Both must authenticate to the same API.
...
The entire Arbitra platform — backend, frontend, smart contracts — lives in a single monorepo.
