...
This document describes the comprehensive alerting rules configured for the Arbitra backend, covering critical errors, performance issues, and business logic failures.
...
kubectl logs -n production -l app=arbitra-backend | grep "CRITICAL"
...
kubectl logs -n production -l app=arbitra-backend | grep -A 5 "Exception"
```
...
kubectl logs -n production -l app=arbitra-backend | grep "timeout"
...
kubectl logs -n production -l app=arbitra-backend | grep "redis"
```
...
kubectl logs -n production -l app=arbitra-backend | grep "payment"
...
kubectl logs -n production -l app=arbitra-backend | grep "stellar"
...
```bash
# Check pod status
kubectl get pods -n production -l app=arbitra-backend
...
# Check health logs
kubectl logs -n production -l app=arbitra-backend | grep "health"
```
...
# Check connection errors
kubectl logs -n production -l app=arbitra-backend | grep "connection"
```
...
kubectl logs -n production -l app=arbitra-backend | grep "escrow"
...
kubectl logs -n production -l app=arbitra-backend | grep "auth"
...
kubectl logs -n production -l app=arbitra-backend | grep "webhook"
