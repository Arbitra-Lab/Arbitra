...
- [ ] **Monthly Backup Success Rate**
  ...
  - Target: > 99.5%
  - Command: Count daily logs: `ls -1 /var/log/arbitra/backup-full.log.* | wc -l`
...
- [ ] **Average Backup Duration Trend**
  ...
  - Target: < 4 hours
  - Command: `grep 'completed' /var/log/arbitra/backup-full.log | grep -oP 'Duration: \K[^s]*' | awk '{sum+=$1; count++} END {print sum/count}'`
  - Action if > 4h: Investigate database growth or resource constraints

- [ ] **WAL Archiving Reliability**
  - Check: `psql -U arbitra -c "SELECT archived_count, failed_count FROM pg_stat_archiver;"`
...
- [ ] **Final Environment Checks**
  ...
  - Verify S3 access: `aws s3 ls s3://arbitra-backups-prod/`
  - Verify PostgreSQL available: `psql --version`
  - Verify scripts accessible: `ls -la /opt/arbitra/scripts/recover-*.sh`

- [ ] **Select Backup for Recovery**
  - Choose backup from 2+ weeks ago: `aws s3 ls s3://arbitra-backups-prod/full/ | grep -v $(date +%Y%m01) | head -1`
  - Document backup name and date
  - Download backup: `aws s3 cp s3://arbitra-backups-prod/full/[backup_date]/ /backups/drill/ --recursive`
  - Verify download completed

- [ ] **Initialize Test Database**
  - Create test database user: `psql -U postgres -c "CREATE USER drill_user WITH PASSWORD 'secure_password';"`
  - Create test database: `psql -U postgres -c "CREATE DATABASE arbitra_monthly_drill;"`
  - Grant permissions: `psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE arbitra_monthly_drill TO drill_user;"`

#### Phase 2: Full Restoration (2-3 hours)

- [ ] **Execute Full Restore**
  - Command: `/opt/arbitra/scripts/recover-full.sh /backups/drill/arbitra_[date].sql.gz arbitra_monthly_drill`
  - Monitor: `tail -f /var/log/arbitra/recovery.log`
...
- [ ] **Verify Restore Completion**
  - Check table count: `psql -U drill_user -d arbitra_monthly_drill -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public';"`
  - Expected: 30+ tables
  - Check data: `psql -U drill_user -d arbitra_monthly_drill -c "SELECT COUNT(*) FROM users; SELECT COUNT(*) FROM properties;"`
  - Expected: Significant row counts matching production

- [ ] **Verify Schema Integrity**
  - Check indices: `psql -U drill_user -d arbitra_monthly_drill -c "SELECT COUNT(*) FROM pg_indexes WHERE schemaname = 'public';"`
  - Check constraints: `psql -U drill_user -d arbitra_monthly_drill -c "SELECT COUNT(*) FROM information_schema.table_constraints;"`
  - Check sequences: `psql -U drill_user -d arbitra_monthly_drill -c "SELECT COUNT(*) FROM information_schema.sequences;"`
...
- [ ] **Users and Accounts**
  ...
  ```bash
  psql -U drill_user -d arbitra_monthly_drill -c \
...
- [ ] **User Authentication Validation**
  ...
  - Command: `psql -U drill_user -d arbitra_monthly_drill -c "SELECT id, email, role FROM users LIMIT 10;"`
...
- [ ] **Financial Data Integrity**
  ...
  - Command: `psql -U drill_user -d arbitra_monthly_drill -c "SELECT COUNT(*) FROM payments WHERE status = 'completed';"`
...
- [ ] **Blockchain/Smart Contract Data**
  ...
  - Command: `psql -U drill_user -d arbitra_monthly_drill -c "SELECT COUNT(*) FROM blockchain_transactions;"`
...
- [ ] **Run Foreign Key Check**
  ...
  ```bash
  psql -U drill_user -d arbitra_monthly_drill << EOF
...
- [ ] **Query Performance Sampling**
  ...
  ```bash
  # Time critical query
  time psql -U drill_user -d arbitra_monthly_drill -c \
...
- [ ] **Clean Up Test Databases**
  - Drop test database: `psql -U postgres -c "DROP DATABASE arbitra_monthly_drill;"`
