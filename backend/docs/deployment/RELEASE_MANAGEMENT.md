...
This document defines release management procedures for the Arbitra backend, covering release planning, versioning strategy, release notes creation, deployment, rollback, and communication.
...
Arbitra follows Semantic Versioning (SemVer): `MAJOR.MINOR.PATCH`
...
https://github.com/org/arbitra/compare/v1.2.2...v1.2.3
```
...
docker build \
  ...
  -t ghcr.io/arbitra/backend:${VERSION} \
  -t ghcr.io/arbitra/backend:latest \
  backend/

# 4. Push image to registry
docker push ghcr.io/arbitra/backend:${VERSION}
docker push ghcr.io/arbitra/backend:latest
...
kubectl set image deployment/arbitra-backend \
  arbitra-backend=ghcr.io/arbitra/backend:${VERSION}
...
# 10. Verify health
curl -f https://api.arbitra.io/health || exit 1
...
API_URL="${1:-https://api.arbitra.io}"
...
kubectl wait --for=condition=available deployment/arbitra-backend-green

# Run smoke tests on green
./scripts/smoke-tests.sh green

# Switch traffic to green
kubectl patch service arbitra-backend -p '{"spec":{"selector":{"version":"green"}}}'

# Monitor for issues
sleep 300

# If successful, scale down blue
kubectl scale deployment/arbitra-backend-blue --replicas=0

# If issues, rollback to blue
# kubectl patch service arbitra-backend -p '{"spec":{"selector":{"version":"blue"}}}'
```
...
kubectl set image deployment/arbitra-backend \
  arbitra-backend=ghcr.io/arbitra/backend:${PREVIOUS_VERSION}
...
echo "Grafana: https://grafana.arbitra.io"
...
// Or via API
curl -X POST https://api.arbitra.io/admin/feature-flags/new-feature/disable \
...
**Dashboard:** https://grafana.arbitra.io/deployment
...
We're excited to announce new features and improvements to Arbitra!
...
Our support team is here to help. Contact us at support@arbitra.io or visit our help center.

Thank you for using Arbitra!
...
# Check recent errors
kubectl logs -l app=arbitra-backend --tail=100 | grep ERROR
    ...
    steps:
      ...
      - name: Build Docker image
        run: |
          docker build -f backend/Dockerfile.production \
            -t ghcr.io/arbitra/backend:${{ github.ref_name }} \
            -t ghcr.io/arbitra/backend:latest \
            backend/
      ...
      - name: Push Docker image
        run: |
          ...
          docker push ghcr.io/arbitra/backend:${{ github.ref_name }}
          docker push ghcr.io/arbitra/backend:latest
