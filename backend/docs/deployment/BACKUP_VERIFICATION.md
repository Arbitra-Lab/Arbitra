...
- Latest `backup_*.sql.gz` exists under `BACKUP_DIR` (default `/var/backups/arbitra`)
- File is readable and passes `gunzip -t`
- File size is at least 1 KB

### Full verification

1. Creates temporary database `arbitra_verify_<timestamp>`
...
| ----------------------------- | --------------------- | --------------------------------- |
| `BACKUP_DIR`                  | `/var/backups/arbitra` | Directory containing backup files |
