...
This Disaster Recovery Plan (DRP) defines the procedures, responsibilities, and recovery objectives for restoring the Arbitra platform following a disruptive event. It covers all production infrastructure including the NestJS backend API, Next.js frontend, PostgreSQL (Neon) database, Redis cache, Stellar blockchain integrations, S3 file storage, and supporting services.

This plan applies to:

- Production environment (`api.arbitra.io`, `app.arbitra.io`)
...
- **Status page:** Update `status.arbitra.io` within 30 minutes of confirmed outage
...
- **Snapshot storage:** Encrypted and stored in S3 (`s3://arbitra-backups/db/`)
...
pg_restore --clean --no-acl --no-owner \
  -d $DATABASE_URL \
  backups/arbitra_$(date +%Y%m%d).dump
```
...
**Symptoms:** `app.arbitra.io` returns 502/503, Vercel/CDN reports deployment failure.
...
# 7. Verify health
curl https://api.arbitra.io/health
curl https://app.arbitra.io
```
