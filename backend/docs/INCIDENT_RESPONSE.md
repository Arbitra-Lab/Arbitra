...
This document defines the end-to-end incident response procedures for the Arbitra platform. It covers how incidents are classified, detected, escalated, communicated, tracked, resolved, and reviewed — plus runbooks for common incidents and strategies for prevention.
...
The Arbitra platform uses automated monitoring to detect incidents before users report them.
...
| -------------------------- | ---------------- | ------------------- |
| Email (support@arbitra.dev) | External users   | 4 hours (per SLA)   |
...
| Channel                         | Purpose                                  | Audience        |
| ------------------------------- | ---------------------------------------- | --------------- |
| Status page (status.arbitra.dev) | Service availability updates             | All users       |
| Email (noreply@arbitra.dev)      | Incident notifications to affected users | Affected users  |
...
| ------------------------------ | ------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| **Rollback deployment**        | Recent deploy caused the incident                | `kubectl rollout undo deployment/arbitra-backend` or redeploy previous image                     |
| **Scale up instances**         | Traffic spike or resource exhaustion             | `kubectl scale deployment arbitra-backend --replicas=N`                                          |
...
All incidents are tracked in the Arbitra incident tracker. Each record must contain:
...
```
10:30:00 — Alert fired: HighErrorRate on arbitra-backend
...
kubectl logs -l app=arbitra-backend --tail=100
...
2. If OOMKilled, increase memory limits:
   ```bash
   kubectl set resources deployment/arbitra-backend -c=app \
     --limits=memory=1Gi
   ```
...
# LogQL: {service="arbitra-backend"} | json | level="error" | __error__=""

# 3. Check Sentry for new error groups
# https://sentry.io/organizations/arbitra/issues/
...
2. If a specific endpoint, consider disabling it temporarily:
   ...
   kubectl set env deployment/arbitra-backend DISABLE_ENDPOINT_<NAME>=true
   ```
...
3. If high CPU, scale up:
   ```bash
   kubectl scale deployment arbitra-backend --replicas=4
   ```
4. If queue backlog, add more workers:
   ```bash
   kubectl scale deployment arbitra-workers --replicas=3
   ```
...
# LogQL: {service="arbitra-backend"} | json | level="warn" | "connection"
```
...
3. Increase pool size if needed (temporary):
   ```bash
   kubectl set env deployment/arbitra-backend DB_POOL_SIZE=20
   ```
...
kubectl rollout undo deployment/arbitra-frontend
```
...
1. **Contain** — Revoke compromised credentials immediately
   ...
   kubectl set env deployment/arbitra-backend JWT_SECRET=<new-secret>
...
# LogQL: {service="arbitra-backend"} | json | "429" | "rate_limit"
```
...
# https://admin.arbitra.io/queues (or local http://localhost:5000/queues)
...
# 4. Check worker logs
kubectl logs -l app=arbitra-workers --tail=200 | grep -i "error"
...
2. If a specific job type is causing failures, pause it:
   ```bash
   kubectl set env deployment/arbitra-workers PAUSE_QUEUE_<NAME>=true
   ```
3. Scale up workers to clear backlog:
   ```bash
   kubectl scale deployment arbitra-workers --replicas=5
   ```
