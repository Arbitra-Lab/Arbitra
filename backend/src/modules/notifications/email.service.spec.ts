...
describe('EmailService', () => {
  ...
  const mockConfig: Record<string, string> = {
    EMAIL_SERVICE: 'gmail',
    EMAIL_USER: 'test@arbitra.app',
    EMAIL_PASSWORD: 'test-password',
    EMAIL_FROM: '"Arbitra App" <noreply@arbitra.app>',
    FRONTEND_URL: 'https://app.arbitra.io',
    PASSWORD_RESET_URL: 'https://app.arbitra.io/reset-password',
  };
  ...
  describe('sendVerificationEmail', () => {
    ...
    it('includes a verification URL pointing to the frontend', async () => {
      ...
      expect(mailOptions.html).toContain(
        'https://app.arbitra.io/verify-email?token=tok123',
      );
    });
    ...
    it('uses configured FROM address', async () => {
      ...
      const mailOptions = sendMailMock.mock.calls[0][0];
      expect(mailOptions.from).toBe('"Arbitra App" <noreply@arbitra.app>');
    });
  ...
  describe('sendPasswordResetEmail', () => {
    ...
    it('builds reset URL from PASSWORD_RESET_URL config', async () => {
      ...
      const mailOptions = sendMailMock.mock.calls[0][0];
      expect(mailOptions.html).toContain(
        'https://app.arbitra.io/reset-password?token=tok',
      );
    });
  ...
  describe('sendNotificationEmail', () => {
    ...
    it('renders action link when data.actionUrl is provided', async () => {
      await service.sendNotificationEmail('n@example.com', 'Action', 'tmpl', {
        title: 'Click',
        message: '',
        actionUrl: 'https://app.arbitra.io/action',
        actionText: 'View Now',
      });
      const mailOptions = sendMailMock.mock.calls[0][0];
      expect(mailOptions.html).toContain('https://app.arbitra.io/action');
  ...
  describe('sendAlertEmail', () => {
    it('sends alert to the given address with correct subject', async () => {
      await service.sendAlertEmail('admin@arbitra.app', 'High CPU Alert', {
        message: 'CPU usage exceeded 90%',
      });
      expect(sendMailMock).toHaveBeenCalledWith(
        expect.objectContaining({
          to: 'admin@arbitra.app',
    ...
    it('includes the alert message in the body', async () => {
      await service.sendAlertEmail('admin@arbitra.app', 'Alert', {
    ...
    it('throws when transporter fails', async () => {
      ...
      await expect(
        service.sendAlertEmail('admin@arbitra.app', 'Alert', { message: 'x' }),
