...
Deploy Arbitra Soroban contracts to [Stellar testnet](https://developers.stellar.org/docs/build/smart-contracts/getting-started/deploy-to-testnet).
...
| `PLATFORM_FEE_BPS` | `500` | Arbitra Agreement `fee_bps` (5%) |
...
**Deploy order:** `user_profile` → `property_registry` → `agent_registry` → `rent_obligation` → `escrow` → `payment` → `dispute_resolution` → `arbitra_agreement`

**Init order:** same through `payment`, then `arbitra_agreement`, then `dispute_resolution` (needs `ARBITRA_AGREEMENT_CONTRACT_ID`).
