...
This document explains how to **credibly demonstrate** that Arbitra’s Soroban contracts are working together as intended **from a testing/CI perspective**, and what evidence belongs in a PR description.
...
- `dispute_resolution` validates disputes against agreements stored in the Arbitra contract via a cross-contract fetch (see `raise_dispute` implementation).
...
Notes:
- Focus: <e.g. dispute_resolution ↔ arbitra agreement fetch + error matrix>
```
