...
In this guide, you will create a simple **Housing Registry** contract from scratch — a miniature version of Arbitra's own `property_registry` contract. By the end, you will understand the anatomy of a Soroban contract, how to build it to WebAssembly, and where the compiled artifacts live.
...
└── target/
    └── wasm32-unknown-unknown/
        └── release/
            ...
            ├── property_registry.wasm     ← Arbitra's property registry
            ├── escrow.wasm                ← Arbitra's escrow contract
...
A typical minimal Soroban contract is between **1 KB and 50 KB**. The Arbitra workspace's `Cargo.toml` already configures aggressive optimizations:
...
Before committing, run the Arbitra quality checks:
...
```bash
stellar keys generate --global arbitra-dev --network testnet
```

Replace `arbitra-dev` with any name you like — this is a local label, not visible on-chain.
...
```bash
stellar keys address arbitra-dev
```
...
```bash
curl "https://friendbot.stellar.org/?addr=$(stellar keys address arbitra-dev)"
```

Or on **Windows (PowerShell)**:

```powershell
$addr = stellar keys address arbitra-dev
...
Replace `YOUR_PUBLIC_ADDRESS_HERE` with the output from `stellar keys address arbitra-dev`.
...
## Understanding the Arbitra Contract Structure
...
As you contribute to Arbitra, you'll follow the production patterns. But the fundamentals — `#[contract]`, `#[contractimpl]`, storage, and types — are exactly what you've learned here.
