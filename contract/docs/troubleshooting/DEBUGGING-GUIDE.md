...
This guide provides step-by-step procedures for debugging Arbitra smart contracts, including local testing, deployment issues, and production troubleshooting.
...
1. **Check contract logs** (if available):
   ...
   tail -f /var/log/arbitra/backend.log | grep contract
   ```
