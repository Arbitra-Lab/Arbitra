...
| `escrow_id` | Escrow lifecycle identifier (`BytesN<32>` in `escrow`, `String` in tokenized arbitra escrow events). |
...
- Normalize event names into `contract.event` convention (for example `arbitra.agreement_created`).
