...
This guide demonstrates how to integrate the Property Registry Contract with the Arbitra Rental Contract to ensure rental agreements are tied to verified properties.

## Overview

The Property Registry Contract provides an on-chain verification layer for properties. Before creating a rental agreement, the Arbitra contract can query the registry to ensure:
...
In the Arbitra contract's `Cargo.toml`, add the property registry as a dependency (if building as separate contracts):
...
Add property-related errors to `contracts/arbitra/src/errors.rs`:
...
Update `contracts/arbitra/src/types.rs` to include the property registry address:
...
In `contracts/arbitra/src/lib.rs`, add a method to configure the property registry:
...
Modify the `create_agreement` function in `contracts/arbitra/src/agreement.rs`:
...
Add the property_id field to the RentAgreement struct in `contracts/arbitra/src/types.rs`:
...
let rental_contract = ArbitraContractClient::new(&env, &rental_contract_id);
