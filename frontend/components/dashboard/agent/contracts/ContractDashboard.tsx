...
export function ContractDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* Page Header */}
      <div>
        <h1 className="text-2xl font-bold text-gradient-verdict">
          Contracts & Legal
        </h1>
        <p className="text-sm text-neutral-200/60 font-medium mt-1">
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 p-5 rounded-2xl border transition-all text-left group ${
                isActive
                  ? 'bg-brand-blue/20 text-white border-brand-blue shadow-[0_0_20px_rgba(37,99,235,0.2)]'
                  : 'bg-white/5 text-neutral-200/40 border-white/10 hover:border-white/20 hover:bg-white/10 shadow-sm'
              }`}
            >
              <div
                className={`p-2.5 rounded-xl border transition-all ${
                  isActive
                    ? 'bg-brand-blue border-brand-blue-dark shadow-lg'
              ...
              >
                <Icon
                  ...
                  className={
                    isActive
                      ? 'text-white'
                      : 'text-brand-accent/60 group-hover:text-brand-accent'
                  }
                />
              </div>
              <div>
                ...
                <span
                  className={`block text-[10px] font-bold uppercase tracking-widest mt-1.5 ${
                    isActive
                      ? 'text-neutral-100'
                      : 'text-neutral-200/40 group-hover:text-neutral-200/60'
                  }`}
                >
                  {tab.label}
                </span>
      ...
      <div className="relative group">
        <Search
          size={18}
          className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-200/40 group-focus-within:text-brand-accent transition-colors"
        />
        <input
          type="text"
          value={searchQuery}
          ...
          className="w-full pl-11 pr-4 py-4 bg-white/5 border border-white/10 rounded-2xl text-sm text-white placeholder:text-neutral-200/20 outline-none focus:border-brand-accent focus:bg-white/10 focus:shadow-[0_0_20px_rgba(37,99,235,0.1)] transition-all"
        />
      </div>
