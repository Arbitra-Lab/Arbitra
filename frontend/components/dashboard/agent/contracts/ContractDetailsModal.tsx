...
function generateContractPdf(contract: Contract): void {
  ...
  const html = `
    ...
    <body>
      ...
      <div class="footer">
        <p>Generated by Arbitra Protocol &bull; ${new Date().toLocaleDateString()}</p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden">
        ...
        <div className="flex items-start justify-between p-8 border-b border-white/5 shrink-0">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 text-xs font-bold text-white bg-brand-blue/50 border border-brand-blue/30 rounded-xl hover:bg-brand-blue hover:border-brand-blue-dark transition-all uppercase tracking-widest shadow-lg"
            >
              <Download size={16} />
              Download PDF
            </button>
            <button
              onClick={onClose}
              className="p-2 text-neutral-200/40 hover:text-white hover:bg-white/5 rounded-xl transition-all"
            >
              <X size={24} />
            </button>
          </div>
        ...
        <div className="px-8 py-6 border-b border-white/5 bg-white/5">
          <p className="text-[10px] font-bold text-neutral-200/40 uppercase tracking-widest mb-6 px-1">
            Contract Lifecycle
          </p>
          ...
          <button
            ...
            className={`pb-4 text-xs font-bold uppercase tracking-widest transition-all border-b-2 ${
              activeTab === 'details'
                ...
                : 'text-neutral-200/40 border-transparent hover:text-neutral-200/60'
            }`}
          >
            Contract Details
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          {activeTab === 'details' && (
            <div className="space-y-8">
              {/* Parties */}
              <div>
                <h3 className="text-xs font-bold text-neutral-200/40 uppercase tracking-widest mb-4">
                  Parties Involved
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                  {[contract.landlord, contract.tenant, contract.agent].map(
                    (party) => (
                      ...
                      >
                        <span className="block text-[10px] font-bold text-neutral-200/40 uppercase tracking-widest mb-2">
                          {party.role}
                        </span>
                        <span className="block text-sm font-bold text-white mb-1 group-hover:text-brand-accent transition-colors">
                          {party.name}
                        </span>
                        <span className="block text-[10px] font-mono text-neutral-200/40 truncate">
                          {party.walletAddress}
                        </span>
                      </div>
                    ),
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
                  <div className="p-5 bg-brand-accent/10 rounded-2xl border border-brand-accent/20 group hover:bg-brand-accent/20 transition-all">
                    <span className="block text-[10px] font-bold text-brand-accent uppercase tracking-widest mb-1.5">
                      Annual Rent
                    </span>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 p-5 bg-white/5 rounded-2xl border border-white/10 group hover:border-white/20 transition-all shadow-inner">
                    <div className="flex-1 min-w-0">
                      ...
                      <code className="text-xs font-mono text-neutral-200/60 break-all">
                        {contract.stellarTxHash}
                      </code>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        ...
                        className="p-2.5 text-neutral-200/40 hover:text-brand-accent hover:bg-white/5 rounded-xl transition-all shadow-sm"
          ...
          {activeTab === 'terms' && (
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8 font-mono text-sm leading-relaxed text-neutral-100/80 whitespace-pre-wrap min-h-[400px] shadow-inner">
              {contract.terms}
            </div>
          )}
        </div>
        ...
        <div className="p-6 border-t border-white/5 bg-white/10 flex items-center justify-between shrink-0 rounded-b-3xl">
          <span className="text-[10px] font-bold text-neutral-200/40 uppercase tracking-widest">
            Created{' '}
          ...
          <button
            onClick={onClose}
            className="px-6 py-2.5 rounded-xl font-bold text-xs text-neutral-200/40 hover:text-white hover:bg-white/5 transition-all uppercase tracking-widest"
          >
            Close
          </button>
        </div>
