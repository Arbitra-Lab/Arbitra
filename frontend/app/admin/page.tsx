...
export default function AdminHomePage() {
  ...
  return (
    <section className="space-y-6">
      ...
      {navItems.length === 0 ? (
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-neutral-200/75">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
          {navItems.map((item) => (
            ...
            >
              <item.icon className="w-8 h-8 text-brand-accent group-hover:text-neutral-200 transition-colors" />
              <h2 className="mt-4 text-lg font-semibold text-white">
                {item.label}
              </h2>
              <p className="mt-1 text-sm text-neutral-200/65">
